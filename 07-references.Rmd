...
# Pilot demographics {#pilotdemo}

## Response date {#responsedate}

As reported by Qualtrics' `Recorded Date` estimates:

```{r pilotdate, echo=FALSE, warning=FALSE, fig.cap="Qualtrics recorded date of pilot survey response"}

fordemo <- read.csv("..\\Engagement-Techreport\\Pilot Qualtrics\\qualtrics_pilot_data.csv", header=FALSE, na.strings="")

y <- fordemo[2,]
demo <- fordemo[-c(1:3),]
colnames(demo) <- y

demo$date <- substr(demo$`Recorded Date`, 1,10)
demo$date <- as.Date(demo$date)

library(ggplot2)

ggplot(demo, aes(x=date)) + geom_histogram(binwidth=1) +
  scale_x_date(labels = scales::date_format("%Y-%b"), breaks = seq(min(demo$date)-5, max(demo$date)+5, 30)) +
  xlab("")
...
As reported by Qualtrics' `Duration (in seconds)` variable:

```{r pilotlatency, echo=FALSE, warning=FALSE, fig.cap="Qualtrics recorded response latencies (truncated for over 7000 seconds)"}


demo$latency <- as.numeric(demo$`Duration (in seconds)`)
demo$latency[demo$latency > 7000] <- NA

aaa <- ggplot(demo, aes(x=latency)) + geom_histogram() + xlab("")
suppressMessages(print(aaa))                                         ## Workaround for package-dependent warning messages

```


## Geographic constituency {#geo}

As reported by Qualtrics' `Location Latitude` and `Location Longitude` estimates, rounded to one decimal place in consideration of pilot respondents' privacy:

```{r geo, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Geographic location of pilot sample respondents"}

library(leaflet)

demo$`Location Longitude` <- round(as.numeric(as.character(demo$`Location Longitude`)),1)
demo$`Location Latitude` <- round(as.numeric(as.character(demo$`Location Latitude`)),1)
...
leaflet() %>% 
  ...
  setView(-98.35, 39.7,
          zoom = 4) %>%
  addTiles() %>%
  addCircles(data=demo, lng = ~`Location Longitude`, lat = ~`Location Latitude`, weight = 10)
...
The average Flesch-Kincaied (e.g., reading grade) for our 36 candidate pilot items was `r round(meanfk,2)` (*sd* = `r round(sdfk,2)`). The average Dale-Chall index was `r round(meandc,2)` (*sd* = `r round(sddc,2)`).
...
ggplot(temp, aes(x = Dale.Chall, fill = combo)) +
  ...
  scale_fill_manual(values=c("#BCA0DC", "#7C5295", "#52307C", "#08F26E", "#06C258", "#059142", "#FFFFB7", "#FFEA61", "#FFD400")) +
...
ggplot(temp, aes(x = Dale.Chall, fill = Substantive)) +
  scale_fill_manual(values=c("#D0EFFF", "#187BCD", "#03254C")) + 
  geom_density(alpha = .5) +
...
ggplot(temp, aes(x = Dale.Chall, fill = Attitudinal)) +
  geom_density(alpha = .5) +
  scale_fill_manual(values=c("#FF8B8E", "#FF0000", "#C80004")) +
...
# CFA Modification Indices {#mods}

Initial CFA modification indices (top 700 reported; 36-item specification):

```{r modindices, echo=FALSE, warning=FALSE, message=FALSE}

substantive <- modindices(Fit1.1, sort = TRUE, maximum.number = 700)
attitudinal <- modindices(Fit1.2, sort = TRUE, maximum.number = 700)

colnames(substantive)[1] <- "sub.element1"
colnames(substantive)[3] <- "sub.element2"
colnames(substantive)[4] <- "chisq.sub"

colnames(attitudinal)[1] <- "att.element1"
colnames(attitudinal)[3] <- "att.element2"
colnames(attitudinal)[4] <- "chisq.att"

sub_sub <- substantive[,c(1,3,4)]
att_sub <- attitudinal[,c(1,3,4)]

allmods <- cbind(sub_sub,att_sub)
allmods$chisq.sub <- round(allmods$chisq.sub, 2)
allmods$chisq.att <- round(allmods$chisq.att, 2)

library(DT)
datatable(allmods, rownames=FALSE, options = list(pageLength = 10))  
