...
As reported by Qualtrics' `Location Latitude` and `Location Longitude` estimates:

```{r geo, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Geographic location of pilot sample respondents"}

library(leaflet)

data$`Location Longitude` <- as.numeric(as.character(data$`Location Longitude`))
data$`Location Latitude` <- as.numeric(as.character(data$`Location Latitude`))
...
leaflet() %>% 
  ...
  setView(-98.35, 39.7,
          zoom = 4) %>%
  addTiles() %>%
  addCircles(data=data, lng = ~`Location Longitude`, lat = ~`Location Latitude`, weight = 10)
